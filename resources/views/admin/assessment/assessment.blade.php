    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                {{-- HEADER FILTER (Disatukan, Tanpa Label, dengan Search) --}}
                    <div class="p-4 sm:p-6 border-b border-gray-200">
                        
                        {{-- SEARCH BAR --}}
                        <div class="w-full mb-4">
                            <form action="{{ route('admin.assessment.index') }}" method="GET" class="relative w-full">
                                {{-- Simpan semua filter yang aktif (selain 'search') sebagai hidden input --}}
                                @foreach(['academic_year', 'category', 'type', 'class_id', 'class_status'] as $key)
                                    {{-- Memastikan filter yang dipilih (termasuk status default 'active') dibawa --}}
                                    @if(request()->has($key)) 
                                        <input type="hidden" name="{{ $key }}" value="{{ request($key) }}"> 
                                    @elseif($key == 'class_status' && !request()->has('class_status'))
                                        {{-- Default class_status ke 'active' jika tidak ada di URL --}}
                        ...
                        <form action="{{ route('admin.assessment.index') }}" method="GET" class="w-full">
                            {{-- Simpan nilai 'search' yang aktif sebagai hidden input --}}
                            @if(request('search')) 
                            ...
                            <tr class="hover:bg-gray-50 transition-colors group">
                                ...
                                <td class="px-6 py-5 whitespace-nowrap text-center">
                                    <div class="flex items-center justify-center gap-3">
                                        ...
                                        {{-- 1. View Grades (Tetap: Mode Default Read-Only) --}}
                                        <a href="{{ route('admin.classes.assessment.detail', [
                                        ...
                                        {{-- 2. Edit Session (MODIFIKASI: Tambah parameter 'mode=edit') --}}
                                        <a href="{{ route('admin.classes.assessment.detail', [
                                                    'classId' => $assessment->class_id, 
                                                    'type' => $assessment->type, 
                                                    'from' => 'assessment',
                                                    'mode' => 'edit' // <-- PARAMETER BARU
                                                ]) }}" 
