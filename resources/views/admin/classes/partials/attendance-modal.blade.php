<div x-show="showStudentStatsModal" 
    x-init="$watch('showStudentStatsModal', value => {
        if (value) {
            // Tunggu elemen render ($nextTick) lalu scroll ke kanan mentok (melihat sesi terbaru)
            $nextTick(() => {
        ...
        <div class="inline-block align-bottom bg-white rounded-xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-6xl w-full">
            ...
            <div class="bg-white px-6 py-4 border-b border-gray-100 flex justify-between items-center sticky top-0 z-50">
                <div>
                    ...
                    <p class="text-sm text-gray-500 mt-1">Showing all sessions breakdown.</p>
                </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead class="bg-gray-50 text-gray-500 text-xs font-bold uppercase border-b border-gray-200 sticky top-0 z-20 shadow-sm">
                        <tr>
                            ...
                            {{-- LOOP HEADER: GANTI DARI $teachingLogs KE $matrixSessions (Urutan Oldest -> Newest) --}}
                            @foreach($matrixSessions as $session)
                    ...
                    <tbody class="divide-y divide-gray-100 text-sm">
                        @foreach($studentStats as $stat)
                            <tr class="transition group 
                                ...
                                <td class="px-4 py-3 sticky left-0 z-10 border-r border-gray-100 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.05)] 
                                    ...
                                    <div class="flex flex-col justify-center h-full">
                                        ...
                                        @if($stat->deleted_at || !$stat->is_active)
                                            <div class="mt-1">
                                                @if($stat->deleted_at)
                                                    <span class="inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-bold bg-red-600 text-white border border-red-700 uppercase tracking-wide">DELETED</span>
                                                    <span class="text-[9px] text-gray-400 ml-1">{{ \Carbon\Carbon::parse($stat->deleted_at)->format('d/m/y') }}</span>
                                                @elseif(!$stat->is_active)
                                                    <span class="inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-bold bg-white text-red-600 border border-red-200 uppercase tracking-wide">OUT</span>
                                                @endif
                                            </div>
                                        @endif
                                        <div class="text-[10px] font-mono opacity-70 mt-0.5">{{ $stat->student_number }}</div>
                                    </div>
                                </td>
                                ...
                                {{-- LOOP ISI CELL: GANTI DARI $teachingLogs KE $matrixSessions AGAR SINKRON DENGAN HEADER --}}
                                @foreach($matrixSessions as $session)
                                    @php
                                        // Ambil ID dari view (biasanya 'id' atau 'session_id')
                                        // Pastikan 'id' ada di view v_class_activity_logs
                                        $sessId = $session->id ?? $session->session_id; 
                                        $status = $attendanceMatrix[$stat->student_id][$sessId] ?? '-';
            ...
            <div class="bg-gray-50 px-6 py-4 border-t border-gray-100 flex justify-end items-center gap-3">
                <a href="{{route('admin.classes.attendanceReport', $class->id)}}" target="_blank" class="px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 shadow-sm transition flex items-center gap-2 decoration-0">
                    <svg class="w-4 h-4 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 00-2 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"></path></svg>
                    Print Report
                </a>
                <button @click="showStudentStatsModal = false" class="px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 shadow-sm transition">Close Report</button>
            </div>
        </div> 
    </div>
</div>