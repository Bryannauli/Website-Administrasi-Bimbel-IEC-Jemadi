@php
    // Definisi Class Styling (Tampilan Lama)
...
<div :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'"
    ...
    <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
        ...
        @if($isAdmin)
            ...
            {{-- 5. SYSTEM TRASH (BARU DITAMBAHKAN) --}}
            <a href="{{ route('admin.trash.index') }}"
               class="group flex items-center px-3 py-2 text-sm font-medium rounded-md transition-colors duration-150 ease-in-out mt-4 border-t border-gray-200 pt-2
               {{ request()->routeIs('admin.trash.index') ? 'bg-red-200 text-red-700' : 'text-gray-700 hover:bg-red-100 hover:text-red-700' }}">
                
                <svg class="mr-3 h-6 w-6 flex-shrink-0
                    {{ request()->routeIs('admin.trash.index') ? 'text-red-700' : 'text-gray-700 group-hover:text-red-700' }}"
                    xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
